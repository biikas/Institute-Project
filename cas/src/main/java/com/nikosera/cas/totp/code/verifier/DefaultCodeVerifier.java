...
public class DefaultCodeVerifier implements CodeVerifier {
    ...
    public DefaultCodeVerifier(CodeGenerator codeGenerator, TimeProvider timeProvider, CasFileConfig casFileConfig) {
        this.codeGenerator = codeGenerator;
        this.timeProvider = timeProvider;
        timePeriod = casFileConfig.getTotp().getTime().getPeriod() * 1000;
        allowedTimePeriodDiscrepancy = casFileConfig.getTotp().getTime().getDiscrepancy();
    }
    ...
    private boolean checkCode(String secret, long counter, String code) {
        try {
            ...
            log.debug("Matching code: {}", actualCode);
